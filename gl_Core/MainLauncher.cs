...
namespace glc_cs
{
	public partial class gl : Form
	{
		...
		{
			// ImageListのイメージを保持する配列
			Image[] images = new Image[imageList0.Images.Count];
			// イメージを配列にコピーする
			imageList0.Images.CopyTo(images);
			// ImageSizeを変更する
			switch (trackBar1.Value)
			{
				case 0:
					imageList0.ImageSize = new Size(8, 8);
					break;
				case 1:
					imageList0.ImageSize = new Size(32, 32);
					break;
				case 2:
					imageList0.ImageSize = new Size(64, 64);
					break;
			}
			// イメージを再度追加する
			imageList0.Images.Clear();
			imageList0.Images.AddRange(images);
			// gameImgListにセットする
